<?php
    ...
    if (isset($_POST['enviarDoacao']) && !empty($_POST['nome']) && !empty($_POST['email']) && !empty($_POST['telefone'])) { 
        ...
        for($i = 0; $i < count($alimento); $i++) {
            $f_alimento = $alimento[$i];
            $f_tipo = $tipo[$i];
            $f_quantidade = $quantidade[$i];
            $cadastrarProduto = $crud->cadastrarProdutos($nome, $email, $finalCelular, $alimento[$i], $tipo[$i], $quantidade[$i], $texto);
        }

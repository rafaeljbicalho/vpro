...
if (isset($_POST['aprovar']) && !empty($_POST['aprovar'])) {
    ...
    $aprovarDoacao = $crud->aprovar_doacao($aprovarId);

}

// select produtos
$produtosArray = $crud->select("SELECT * FROM Produto WHERE (aprovado = 0 OR aprovado is null)");

// select produtos where aprovados = 1
$produtosAprovadosArray = $crud->select("SELECT * FROM Produto WHERE aprovado = 1");
                ...
                <thead>
                    ...
                    <?php
                        ...
                        <tr>
                            ...
                            <td>
                            <!-- whatsapp link -->
                            WhatsApp link: <a target="_blank" href="https://api.whatsapp.com/send?phone=55<?= $produto['Telefone']; ?>"><?= masc_tel($produto['Telefone']); ?></a>
                            </td>
                            <td><?= (!empty($produto['Mensagem']) ? $produto['Mensagem'] : '-');  ?></td>
                            <td><?= $produto['Item'];  ?></td>
                            <td><?= $produto['Tipo'];  ?></td>
                            <td><?= $produto['Qtd'];   ?></td>
                            <td><?= (empty($produto['Aprovado']) ? '<button type="submit" name="aprovar" class="btn btn-outline-success aprovar" value="'.$produto['ProdutoId'].'">Aprovar</button>' : 'Aprovado'); ?></td>
                        </tr>
                    <?php endforeach ?>
                </tbody>
            </table>
            ...
            <table class="table table-bordered">
                <thead>
                    ...
                    <?php
                        ...
                        foreach($produtosAprovadosArray as $produto) : 
                        ...
                        <tr>
                            ...
                            <td><?= $produto['Tipo'];  ?></td>
                            <td><?= $produto['Qtd'];   ?></td>
                            <td><?= ($produto['Aprovado'] == 1 ) ? '<span class="badge bg-success">Aprovado</span>' : 'Aprovado'; ?></td>
                        </tr>
                    <?php endforeach ?>
                </tbody>
            </table>
